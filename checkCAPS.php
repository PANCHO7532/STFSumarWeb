...
if(!isset($_POST['psw'])) {
	echo "<title>Invalid Password</title><fieldset><legend>Invalid Password</legend><p>Por favor ingresa la contrase�a de una forma correcta.</p></fieldset>";
	die;
}
...
if($configinc['usemysqli'] == "active") {
	...
	if($resultadomysql['userid'] == $caps AND $resultadomysql['password'] == $psw) {
		...
		echo "<head><title>ERROR</title><fieldset><legend>Credentials Error</legend><p>El usuario o contrase�a ingresados son incorrectos.</p></fieldset>";
		die;
	} 
	*/
	...
	if($resultadomysql['userid'] == $caps AND $resultadomysql['password'] == $psw) {
		...
		error_reporting(0);
		echo "<head><meta http-equiv='Content-Type' content='text/html; charset=UTF-8' /><title>ERROR</title><fieldset><legend>Credentials Error</legend><p>El usuario o contraseña ingresados son incorrectos.</p></fieldset>";
		die;
	} 
